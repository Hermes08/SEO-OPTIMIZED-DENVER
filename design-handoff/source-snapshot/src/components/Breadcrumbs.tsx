  ...
  return (
    <>
      <nav aria-label="Breadcrumb" className="flex items-center space-x-2 text-sm text-gray-400 mb-8 overflow-x-auto whitespace-nowrap">
        <Link href="/" className="hover:text-white flex items-center gap-1 focus:outline-none focus:text-orange-500">
        ...
        {items.map((item, index) => (
          <span key={index} className="flex items-center">
            ...
            {index === items.length - 1 ? (
              ...
              <Link href={item.url} className="hover:text-white transition-colors focus:outline-none focus:text-orange-500">
                {item.name}
              </Link>
            )}
          </span>
      ...
      {/* SSR Safe SchemaMarkup is already used here */}
